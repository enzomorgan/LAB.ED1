...
void addIndex(float x, float y, int index){  //função para adicionar valores em determinadas posições
...
void removeIndex(int index){  //função para remover elementos em determinadas posições
	if(index > length || length == 0){  //se a variável for maior que o tamanho da lista e a lista for igual a zero
		printf("Posição inválida ou Lista vazia!!!\n");  //imprimindo
	}
	else{
		if(index == 0){  //se a variável for igual a zero
			listaPontos = listaPontos->prox; //apontando para o próximo
		}
		else{
			Ponto *listaAux = listaPontos; //ponteiro auxiliar
			int i = 0;  //variável contador iniciando em zero
			
			while(i!=index-1){  //enquanto a variável contador for diferente da variável "index" menos um
 				listaAux = listaAux->prox; //apontando para o próximi
 				i++; //incrementação
			}
			listaAux->prox = listaAux->prox->prox; //apontando para o próximo do próximo  
		}
		length--;  //removendo elemento da lista
	}
}
...
int main(){
	...
	removeIndex(0);  //chamando a função para remover 
